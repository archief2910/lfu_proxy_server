...
#define MAX_SIZE 2048										// size of the cache
...
struct cache_element
{
	...
	int len;
	char *url;
	int frequency;				// frequency of access for LFU
	time_t last_accessed; // timestamp for tie-breaking in LFU
	cache_element *next;
};
...
int connectRemoteServer(char *host_addr, int port_num)
{
	...
	{
		cout << "error connecting to remote server" << endl;
		return -1;
	}
	// free(host_addr);
	return remoteSocket;
...
#define MAX_FREQUENCY 1000 // Arbitrary limit for simplicity

cache_element *frequency_list[MAX_FREQUENCY];
...
cache_element *find(char *url)
{
	...
	// Search through all frequency levels
	for (int freq = 1; freq < MAX_FREQUENCY; ++freq)
	{
		cache_element *curr = frequency_list[freq];
		while (curr)
		{
			if (strcmp(curr->url, url) == 0)
			{
				// Cache hit
				curr->frequency++;
				curr->last_accessed = std::time(nullptr);

				// Move element in frequency list
				cache_element *next_elem = curr->next; // Save next element
				frequency_list[freq] = next_elem;			 // Update the frequency list

				if (curr->frequency < MAX_FREQUENCY)
				{
					curr->next = frequency_list[curr->frequency]; // Update next
					frequency_list[curr->frequency] = curr;				// Move to new frequency
				}
				else
				{
					// If frequency exceeds max, we could handle it here
					// e.g., just keep it at max frequency
				}
				...
				std::cout << "Cache hit for URL: " << url << " | Retrieval time: " << duration << " microseconds\n";
				return curr;
			}
			curr = curr->next;
		}
	...
	auto end_time = std::chrono::high_resolution_clock::now();
	auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time).count();
	std::cout << "Cache miss for URL: " << url << " | Retrieval time: " << duration << " microseconds\n";
	return nullptr;
}
...
int add_cache_element(char *data, int size, char *url)
{
	...
	while (current_cache_size + size > MAX_SIZE)
	{
	...
	new_elem->len = size;
	new_elem->url = strdup(url);
	new_elem->frequency = 1;
	new_elem->last_accessed = std::time(nullptr);
	new_elem->next = frequency_list[1];
	frequency_list[1] = new_elem;
	...
	auto end_time = std::chrono::high_resolution_clock::now();
	auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time).count();
	std::cout << "Added to cache URL: " << url << " | Cache size: " << current_cache_size << " | Addition time: " << duration << " microseconds\n";
	return 0;
}
...
void remove_cache_element()
{
	...
	for (int freq = 1; freq < MAX_FREQUENCY; ++freq)
	{
		if (frequency_list[freq])
		{
			cache_element *lfu_elem = frequency_list[freq];
			frequency_list[freq] = lfu_elem->next;

			current_cache_size -= lfu_elem->len;
			free(lfu_elem->data);
			free(lfu_elem->url);
			free(lfu_elem);
			break;
	...
	auto end_time = std::chrono::high_resolution_clock::now();
	auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time).count();
	std::cout << "Removed an element from cache | Removal time: " << duration << " microseconds\n";
}

...
cache_element* find(char* url) {
    ...
    while (curr) {
        if (strcmp(curr->url, url) == 0) {
            ...
            frequency_list[curr->frequency - 1] = curr->next;
            curr->next = frequency_list[curr->frequency];
            frequency_list[curr->frequency] = curr;
...
int add_cache_element(char* data, int size, char* url) {
    if (size > MAX_ELEMENT_SIZE) return -1;
    ...
    new_elem->last_accessed = time(NULL);
